...
export class PublisherSenderRequestController {
  ...
  async createRabbitMqSender(@Body() createPublisherSenderRequestDto: CreatePublisherSenderRequestDto): Promise<Observable<any>> { //: Promise<Observable<any>> {
    ...
    try {
      ...
      createPublisherSenderRequestDto.id = await String(
      ...
      createPublisherSenderRequestDto.create_at = await new Date() // Date.now()
      ...
      if (
        ...
        let process1: any = await this.amqpConnection.publish(createPublisherSenderRequestDto.exchange, 'publisher-sender-request-service', {
          msg: JSON.stringify(req_body),
        }).then((res: any) =>
          // process1 = lastValueFrom(res, { defaultValue: 'empty' })
          console.log(res)
        );

        process1 = await lastValueFrom(process1, { defaultValue: 'empty' })  // promise/toPromise() rxjs v7++

        let process2: any = await this.amqpConnection.publish(createPublisherSenderRequestDto.exchange, 'subscriber-receiver-response-service', {
          msg: JSON.stringify(req_body),
        }).then((res: any) => console.log(res));
        process2 = await lastValueFrom(process2, { defaultValue: 'empty' }) // promise/toPromise() rxjs v7++
        ...
        let process_await: any = null
        let process1: any = await this.amqpConnection.publish(createPublisherSenderRequestDto.exchange, createPublisherSenderRequestDto.routing_key, {
          msg: JSON.stringify(req_body),
        }).then(() => {
          // process_await = lastValueFrom(res, { defaultValue: 'empty' })
          // if (res) {
          res_json.statusCode = 1
          res_json.req_body = createPublisherSenderRequestDto
